...
export class AgentCoreStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: AgentCoreStackProps) {
    ...
    // Create S3 bucket for skills and upload skill files
    const skillsBucket = new s3.Bucket(this, 'SkillsBucket', {
      bucketName: `${spec.name}-skills-${this.account}-${this.region}`,
      removalPolicy: RemovalPolicy.DESTROY,
      autoDeleteObjects: true,
    });

    // Upload the aws-news skill from the project's skills directory
    // __dirname at runtime = .../agentcore/cdk/dist/lib
    // skills dir = .../aws-news-agent/skills (6 levels up from dist/lib)
    const skillsDir = path.resolve(__dirname, '..', '..', '..', '..', '..', '..', 'skills');
    new s3deploy.BucketDeployment(this, 'SkillsDeployment', {
      sources: [s3deploy.Source.asset(skillsDir)],
      destinationBucket: skillsBucket,
      destinationKeyPrefix: 'skills',
    });

    // Grant the harness execution role read/write access to the skills bucket
    for (const harnessRole of this.application.harnessRoles.values()) {
      skillsBucket.grantReadWrite(harnessRole.role);
    }

    new CfnOutput(this, 'SkillsBucketName', {
      description: 'S3 bucket containing agent skills',
